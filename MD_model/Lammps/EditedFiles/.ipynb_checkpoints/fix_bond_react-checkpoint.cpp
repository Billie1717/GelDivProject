...
/* ----------------------------------------------------------------------
Contributing Author: Jacob Gissinger (jacob.r.gissinger@gmail.com) . Modified by Ivan Palaia and Christian Vanhille Campos in November 2022 to June 2023. Modified by Billie Meadowcroft October 2023-December 2023
------------------------------------------------------------------------- */
  ...
  Fix(lmp, narg, arg)
{
  ...
  memory->create(overlapsq2,nreacts,"bond/react:overlapsq2");
      ...
      if (strcmp(arg[iarg],"prob") == 0) {
        ...
        while (iarg < narg && strcmp(arg[iarg],"react") != 0 ) {
          if (strcmp(arg[iarg],"fit") == 0) {
            ...
            else if (strcmp(arg[iarg+1],"xwalls") == 0) {
              modify_create_nucrand[rxn] = utils::numeric(FLERR,arg[iarg+2],false,lmp); // walls of nucleation on x-wall hi & lo -- Billie 23-10-23
              iarg += 1;
            }
            else if (strcmp(arg[iarg+1],"asters") == 0) {
              modify_create_nucrand[rxn] = utils::numeric(FLERR,arg[iarg+2],false,lmp); // nucleation asters -- Billie 04-01-23
              iarg += 1;
            }
            else if (strcmp(arg[iarg+1],"asters2nd") == 0) {
              modify_create_nucrand[rxn] = utils::numeric(FLERR,arg[iarg+2],false,lmp); // nucleation 2nd set of asters -- Billie 06-03-24
              iarg += 1;
            }
            else if (strcmp(arg[iarg+1],"xwallsMiddle") == 0) {
              modify_create_nucrand[rxn] = 1000; // nucleation inner x-walls -- Billie 10-02-25
            }
            else if (strcmp(arg[iarg+1],"xwallsAsters") == 0) {
              modify_create_nucrand[rxn] = 2000; // nucleation inner x-walls -- Billie 12-02-25
            }
            else if (strcmp(arg[iarg+1],"DiffusorsMidzone") == 0) {
              modify_create_nucrand[rxn] = 3000; // nucleation inner x-walls -- Billie 05-03-25
            }
            iarg += 2;
          } else if (strcmp(arg[iarg],"overlap") == 0) {
            if (iarg+3 > narg) error->all(FLERR,"Illegal fix bond/react command: "
                                          "'modify_create' has too few arguments");
            overlapsq[rxn] = utils::numeric(FLERR,arg[iarg+1],false,lmp); //Currently takes the raw value - want to change this to percentage of (sigma1+sigma2)/2 Billie 14/12/2023
            overlapsq2[rxn] = utils::numeric(FLERR,arg[iarg+2],false,lmp); //Added another overlap value for a different atom type Billie 15/12/2023
              
            overlapsq[rxn] *= overlapsq[rxn]; //making it squared value
            overlapsq2[rxn] *= overlapsq2[rxn]; 
            iarg += 3;
          } else break;
        }
      } else error->all(FLERR,"Illegal fix bond/react command: unknown keyword bil");
    }
  }
                                 ...
                                 const std::string& fragid)
{
  ...
  if (rxnfunc == "rxndiffIvan") {
    if (fragid == "all") {
      error->one(FLERR,"Fix bond/react: Molecule fragment "
        ...
        if (onemol->fragmentmask[ifrag][i]) {
          ...
          if (nsum == 0) {
            ...
            error->one(FLERR,"Bond/react: Molecule fragment "
                              "in rxndiffIvan must contain <= 2 atoms");
          }
        }
      //if (iatom1>iatom2)
      //  sumvvec = -sumvvec
...
void FixBondReact::update_everything()
{
  ...
  for (int pass = 0; pass < 2; pass++) {
    ...
    if (force->angle) {   // Ivan
      ...
      for (int i = 0; i < update_num_mega; i++) {
        rxnID = update_mega_glove[0][i];
        onemol = atom->molecules[unreacted_mol[rxnID]]; // Ivan
        twomol = atom->molecules[reacted_mol[rxnID]];
        if ( !(onemol->angleflag) && !(twomol->angleflag) ) // Ivan
          continue;                                         // Ivan
        for (int j = 0; j < twomol->natoms; j++) {
          int jj = equivalences[j][1][rxnID]-1;
          if (atom->map(update_mega_glove[jj+1][i]) < nlocal && atom->map(update_mega_glove[jj+1][i]) >= 0) {
    ...
    if (force->dihedral) {  // Ivan
      ...
      for (int i = 0; i < update_num_mega; i++) {
        ...
        onemol = atom->molecules[unreacted_mol[rxnID]];  // Ivan
        twomol = atom->molecules[reacted_mol[rxnID]];
        if ( !(onemol->dihedralflag) && !(twomol->dihedralflag) )  // Ivan
          continue;                                                // Ivan
        for (int j = 0; j < twomol->natoms; j++) {
          int jj = equivalences[j][1][rxnID]-1;
          if (atom->map(update_mega_glove[jj+1][i]) < nlocal && atom->map(update_mega_glove[jj+1][i]) >= 0) {
...
int FixBondReact::insert_atoms(tagint **my_update_mega_glove, int iupdate)
{
  ...
  if (ifit >= 0 && ifit < atom->nlocal) {
    ...
    for (int j = 0; j < twomol->natoms; j++) {
      ...
      if (!create_atoms[j][rxnID] && !delete_atoms[ipre][rxnID]) {
        ...
        if (modify_create_nucrand[rxnID] == 1) {
          //printf("Got to modify_create_nucrand[rxnID] == 1");
          double ang = 2*M_PI*random[rxnID]->uniform(); // random angle (from individual reaction RNG) - Chris 26/09/2023
          double phi = 2*M_PI*random[rxnID]->uniform(); // random angle (from individual reaction RNG) - Billie 23/10/23
          //printf("%s %d %s %d\n","rxnID",&rxnID,"fit_incr",&fit_incr);
        if (fit_incr == 0) {    
            //printf("%s %d %s %d\n","Other rxnID",&rxnID,"fit_incr",&fit_incr);// 1st template particle, define random position :D Use individual reaction random number generator random[rxnID]
            ...
            xfrozen[fit_incr][2] = (domain->boxhi[2] - domain->boxlo[2]) * (random[rxnID]->uniform()-0.5); //Added option for 3rd dimension - Billie 23/10/2023
            //printf("Got to post xfrozen[fit_incr][2] =");
            if (dimension ==2){ 
            xfrozen[fit_incr][2] = 0.0;}
          }
          else {
            xfrozen[fit_incr][0] = xfrozen[0][0] + (float)fit_incr*cos(ang)*sin(phi);
            xfrozen[fit_incr][1] = xfrozen[0][1] + (float)fit_incr*sin(ang)*sin(phi);
            xfrozen[fit_incr][2] = xfrozen[0][2] + (float)fit_incr*cos(phi); //Added option for 3rd dimension - Billie 23/10/2023 -- CURRENTLY NOT RIGHT--> NEED TO USE FULL POLAR COORDINATES! 
            if (dimension ==2){
        ...
        else if (modify_create_nucrand[rxnID] > 1 && modify_create_nucrand[rxnID] <=2) { // Sample normal distribution in Y (with standard deviation modify_create_nucrand[rxnID]) for new position -- Chris 28/07/2023
          double ang = 2*M_PI*random[rxnID]->uniform(); // random angle (from individual reaction RNG) - Chris 26/09/2023
          double phi = 2*M_PI*random[rxnID]->uniform(); 
          if (fit_incr == 0) {                          // 1st template particle, define random position :D Use individual reaction random number generator random[rxnID] - Sample normal distribution in Y (with standard deviation modify_create_nucrand[rxnID]) for new position -- Chris 28/07/2023
            ...
            xfrozen[fit_incr][0] = xfrozen[0][0] + (float)fit_incr*cos(ang)*sin(phi);
            xfrozen[fit_incr][1] = xfrozen[0][1] + (float)fit_incr*sin(ang)*sin(phi);
            xfrozen[fit_incr][2] = xfrozen[0][2] + (float)fit_incr*cos(phi);
            if (dimension ==2){
            xfrozen[fit_incr][0] = xfrozen[0][0] + (float)fit_incr*cos(ang);
            xfrozen[fit_incr][1] = xfrozen[0][1] + (float)fit_incr*sin(ang); 
            xfrozen[fit_incr][2] = 0.0;}
        ...
        else if (modify_create_nucrand[rxnID] > 2 &&  modify_create_nucrand[rxnID]!=1000 &&  modify_create_nucrand[rxnID]!=2000 &&  modify_create_nucrand[rxnID]!=3000) { // Put new positions as two spherical asters separated by argument  -- Billie 04/01/24 // Edit such that nucleation is bounded by an ellipsoid 17/06/2024
          double s = 1;
          double u1 = random[rxnID]->uniform();
          double Pdist = modify_create_nucrand[rxnID];
          double ang = (float)(2 * M_PI * random[rxnID]->uniform());
          double phi = (float)(M_PI * random[rxnID]->uniform());
          double CircRadius = (float)5.0; //this is the radius of the sphere we are placing our asters on
          if ((u1) < 0.5){s = 1.0;}
          else{s = -1.0;}
          //double ang = (float)M_PI;
          if (fit_incr == 0) {
            xfrozen[fit_incr][0] = (float)(CircRadius*sin(phi)*cos(ang)) - (float)(2*cos(ang)*sin(phi))+ s*Pdist*0.5;
            xfrozen[fit_incr][1] = (float)(CircRadius*sin(phi)*sin(ang)) - (float)(2*sin(ang)*sin(phi));
            xfrozen[fit_incr][2] = (float)(CircRadius*cos(phi)) - (float)(2*cos(phi));
            if (dimension ==2){
                xfrozen[fit_incr][0] =(float)CircRadius * cos(ang) + s*Pdist*0.5;
                xfrozen[fit_incr][1] = (float)CircRadius * sin(ang);
                xfrozen[fit_incr][2] = 0.0;
            }
          }
          else {
            xfrozen[fit_incr][0] = xfrozen[0][0] + (float)(cos(ang)*sin(phi)); // xfrozen[fit_incr-1][0] + (float)(cos(ang)*sin(phi));
            xfrozen[fit_incr][1] = xfrozen[0][1] + (float)(sin(ang)*sin(phi));
            xfrozen[fit_incr][2] = xfrozen[0][2] + (float)(cos(phi));
            if (dimension ==2){
            xfrozen[fit_incr][0] = xfrozen[0][0] + (float)(fit_incr-2)*cos(ang);
            xfrozen[fit_incr][1] = xfrozen[0][1] + (float)(fit_incr-2)*sin(ang); 
            xfrozen[fit_incr][2] = 0.0;}
          }
        }
        else if (modify_create_nucrand[rxnID] < -3) { // Put new positions as 2nd dividing cells Billie 06-03-24
          double s = 1;
          double s2 = 1;
          double u1 = random[rxnID]->uniform();
          double u2 = random[rxnID]->uniform();
          ...
          if (fit_incr == 0) {
            xfrozen[fit_incr][0] = (float)(CircRadius*sin(phi)*cos(ang)) - (float)(2*cos(ang)*sin(phi))+ s*Pdist*0.5;
            xfrozen[fit_incr][1] = (float)(CircRadius*sin(phi)*sin(ang)) - (float)(2*sin(ang)*sin(phi)) + s2*Pdist*(0.5/sqrt(2));
            xfrozen[fit_incr][2] = (float)(CircRadius*cos(phi)) - (float)(2*cos(phi));
            if (dimension ==2){
                xfrozen[fit_incr][0] =(float)CircRadius * cos(ang) + s*Pdist*0.5;
                xfrozen[fit_incr][1] = (float)CircRadius * sin(ang) + s2*Pdist*(0.5/sqrt(2));
                xfrozen[fit_incr][2] = 0.0;
            }
          }
          else {
        ...
        else if (modify_create_nucrand[rxnID] == -2) { // Put new positions on either end of box in the x-direction.  -- Billie 23/10/2023
          //printf("Got to modify_create_nucrand[rxnID] == -2\n");
         //   printf("%s %d %s %d\n","rxnID",&rxnID,"fit_incr",&fit_incr);
          double s = 1;
          double u1 = random[rxnID]->uniform();
          if ((u1) < 0.5){s = 1.0;}
          ...
          if (fit_incr == 0) {                          // 1st template particle, define random position :D Use individual reaction random number generator random[rxnID] - Sample normal distribution in Y (with standard deviation modify_create_nucrand[rxnID]) for new position -- Chris 28/07/2023
            //double u1 = random[rxnID]->uniform(); //drand48(rxnID); 
            //double u2 = random[rxnID]->uniform();
            if (s > 0){
                xfrozen[fit_incr][0] = domain->boxlo[0]; //(domain->boxhi[0] - domain->boxlo[0]) * (random[rxnID]->uniform()-0.5);
            }
            else{
                xfrozen[fit_incr][0] = (domain->boxhi[0]); //(domain->boxhi[0] - domain->boxlo[0]) * (random[rxnID]->uniform()-0.5);
                //ang = 0;
                //phi = M_PI*0.5;
                //s = -1.0;
            }
            ...
            xfrozen[fit_incr][1] = (domain->boxhi[1] - domain->boxlo[1]) * (random[rxnID]->uniform()-0.5) * 0.9;
            xfrozen[fit_incr][2] = (domain->boxhi[2] - domain->boxlo[2]) * (random[rxnID]->uniform()-0.5) * 0.9;
          }
          else {
            xfrozen[fit_incr][0] = xfrozen[0][0] + (float)fit_incr*s;
            xfrozen[fit_incr][1] = xfrozen[0][1];
            xfrozen[fit_incr][2] = xfrozen[0][2];
            if (dimension ==2){
            xfrozen[fit_incr][0] = xfrozen[0][0] + (float)fit_incr*s;
        ...
        else if (modify_create_nucrand[rxnID] == -3) { // Put new positions on one end of box in the x-direction.  -- Billie 13/12/2023
          double s = 1;
          if (fit_incr == 0) {                          // 1st template particle, define random position :D 
    
            xfrozen[fit_incr][0] = domain->boxlo[0]+ (float)fit_incr*1.1; 
        ...
        else if (modify_create_nucrand[rxnID] == 2000) { // New nucleation for 'xwalls' in the middle of box.  -- Billie 10/02/2025
          double s = 1;
          double FirstSpindX, SecondSpindX;
          double u1 = random[rxnID]->uniform(); //'s' is which end of box we're nucleating.
          FirstSpindX = (domain->boxhi[0]-domain->boxlo[0])*(0.4) + domain->boxlo[0]; //CHOSEN here that the poles are 1/3 in, and are 1/3 long. Change if needed!  -- Billie 10/02/2025
          SecondSpindX = (domain->boxhi[0]-domain->boxlo[0])*(1-0.4) + domain->boxlo[0];
          if (fit_incr == 0) {  
            if (u1 < 0.5){ //FIRST SPINDLE, POINTING OUT
                xfrozen[fit_incr][0] = FirstSpindX;
                counterSpind = -2;
                AngularPart = 1;
            }
            else { //SECOND SPINDLE, POINTING OUT
                xfrozen[fit_incr][0] = SecondSpindX;
                counterSpind = 2;
                AngularPart = 1;
            }
            theta = (float)(2 * M_PI * random[rxnID]->uniform());
            phirand = (float)(M_PI * random[rxnID]->uniform()); // to implement half-asters : M_PI * 0.65 * random[rxnID]->uniform())
            xfrozen[fit_incr][1] = (domain->boxhi[1] - domain->boxlo[1])*0.07*cos(theta)*(sin(phirand)); //making random positions on a circle radius 0.5*box
            xfrozen[fit_incr][2] = (domain->boxhi[2] - domain->boxlo[2])*0.07*sin(theta)*(sin(phirand));
          }
          else{
            xfrozen[fit_incr][0] = xfrozen[0][0] + (float)(fit_incr)*counterSpind*(AngularPart-1)*(-1) + (float)(fit_incr)*AngularPart*cos(phirand)*counterSpind*0.5;
            xfrozen[fit_incr][1] = xfrozen[0][1] + (float)(fit_incr)*AngularPart*cos(theta)*(sin(phirand));
            xfrozen[fit_incr][2] = xfrozen[0][2] + (float)(fit_incr)*AngularPart*sin(theta)*(sin(phirand));
            if (dimension ==2){
            xfrozen[fit_incr][0] = xfrozen[0][0] + (float)(fit_incr)*counterSpind;
            xfrozen[fit_incr][1] = xfrozen[0][1]; 
            xfrozen[fit_incr][2] = 0.0;}
          }
        }
        else if (modify_create_nucrand[rxnID] == 1000) { // New nucleation for 'xwalls' in the middle of box.  -- Billie 10/02/2025
          double s = 1;
          double FirstSpindX, SecondSpindX;
          double u1 = random[rxnID]->uniform(); //'s' is which end of box we're nucleating.
          FirstSpindX = (domain->boxhi[0]-domain->boxlo[0])*(0.4) + domain->boxlo[0]; //CHOSEN here that the poles are 1/3 in, and are 1/3 long. -- Billie 10/02/2025
          SecondSpindX = (domain->boxhi[0]-domain->boxlo[0])*(1-0.4) + domain->boxlo[0];
          if (fit_incr == 0) {  
            if (u1 > 0.5){ //FIRST SPINDLE, POINTING IN
                xfrozen[fit_incr][0] = FirstSpindX;
                counterSpind = 1;
                AngularPart = 0;
            }
            else{ //SECOND SPINDLE, POINTING IN
                xfrozen[fit_incr][0] = SecondSpindX;
                counterSpind = -1;
                AngularPart = 0;
            }
            theta = (float)(2 * M_PI * random[rxnID]->uniform());
            phirand = (float)(M_PI * random[rxnID]->uniform());
            xfrozen[fit_incr][1] = (domain->boxhi[1] - domain->boxlo[1])*0.1*cos(theta)*(sin(phirand)); //making random positions on a circle radius 0.5*box
            xfrozen[fit_incr][2] = (domain->boxhi[2] - domain->boxlo[2])*0.1*sin(theta)*(sin(phirand));
          }
          else{
            xfrozen[fit_incr][0] = xfrozen[0][0] + (float)(fit_incr)*counterSpind*(AngularPart-1)*(-1) + (float)(fit_incr)*AngularPart*cos(phirand)*counterSpind*0.5;
            xfrozen[fit_incr][1] = xfrozen[0][1] + (float)(fit_incr)*AngularPart*cos(theta)*(sin(phirand));
        ...
        else if (modify_create_nucrand[rxnID] == 3000) { // These are my diffusors nucleating in the midzone  -- Billie 05/03/2025
          double s = 1;
          double MiddleX, MiddleY, MiddleZ;
          //double u1 = random[rxnID]->uniform(); //'s' is which end of box we're nucleating.
          MiddleX = (domain->boxhi[0]-domain->boxlo[0])*(0.5) + domain->boxlo[0]; 
          MiddleY = (domain->boxhi[1]-domain->boxlo[1])*(0.5) + domain->boxlo[1]; 
          MiddleZ = (domain->boxhi[2]-domain->boxlo[2])*(0.5) + domain->boxlo[2]; 
          //SecondSpindX = (domain->boxhi[0]-domain->boxlo[0])*(1-0.4) + domain->boxlo[0];
          xfrozen[fit_incr][0] = MiddleX;
          xfrozen[fit_incr][1] = MiddleY;
          xfrozen[fit_incr][2] = MiddleZ;
    ...
    /*if (modify_create_nucrand[rxnID] >2){
        for (int m = 0; m < twomol->natoms; m++) {
            coords[m][0]=xfrozen[m][0];
            coords[m][1]=xfrozen[m][1];
            coords[m][2]=xfrozen[m][2];}
    }*/
    for (int i = 0; i < 3; i++)
      for (int j = 0; j < 3; j++)
  ...
  if (overlapsq[rxnID] > 0) {
    double overlapR = overlapsq[rxnID];
    ...
    for (int m = 0; m < twomol->natoms; m++) {
      ...
      //coordx = coords[m][2]
      if (create_atoms[m][rxnID] == 1) {
        //if (((coords[m][0]*coords[m][0])/(100*100)+(coords[m][1]*coords[m][1])/(70*70)+(coords[m][2]*coords[m][2])/(70*70)) > 0.95) { //Billie 20/04/2025 IF you want your MTs bounded by an ellipse, Uncomment these lines if this is aster configuration, make the aster bounded by an ellipsoid Billie 17/06/2024 equation of ellipsoid (modify_create_nucrand[rxnID] > 2 -- but have to just do generally 02/09/24 Changed to dimensions 130x90x90 from 100x70x70
           // abortflag = 1;
           // break;
       // } 
        for (int i = 0; i < nlocal; i++) {
          itype = type[i]; //Adding an extra 'R' value for atoms of a different type Billie 14/12/23
          if (itype == 7){overlapR = overlapsq2[rxnID];}
          if (itype == 9){overlapR = overlapsq2[rxnID]*4;} //super obscure, added By Billie 01/05/2024 for some very specific experiments on size sorting
          if (itype == 10){overlapR = overlapsq2[rxnID]*9;}
          if (itype == 11){overlapR = overlapsq2[rxnID]*16;}
          else{overlapR = overlapsq[rxnID];}//Really rubbish way of adding this ... Billie 14/12/23
        ...
        //if (modify_create_nucrand[rxnID] == -3 && coords[m][0] <(domain->boxlo[0]+1) ) { //Billie - For fixed wall 16/04/2024
          // printf("rsq = %.2f ([%.2f, %.2f, %.2f]) < coffsq = %.2f -> abort! -- atom involved: %d\n", rsq, delx, dely, delz, overlapsq[rxnID], atom->tag[i]); -- Chris, 21/02/2023
          //abortflag = 1;
          //break;
        //}
        //if (modify_create_nucrand[rxnID] == -3 && coords[m][0] >(domain->boxhi[0]-1) ) { //Billie - For fixed wall 16/04/2024
          // printf("rsq = %.2f ([%.2f, %.2f, %.2f]) < coffsq = %.2f -> abort! -- atom involved: %d\n", rsq, delx, dely, delz, overlapsq[rxnID], atom->tag[i]); -- Chris, 21/02/2023
          //abortflag = 1;
          //break;
        //}
